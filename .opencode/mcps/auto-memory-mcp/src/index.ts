...
class AutoMemorySystem {
  ...
  private isDarkKnowledge(message: string): { isDark: boolean; reason: string } {
    ...
    const darkPatterns: { pattern: RegExp; reason: string }[] = [
  ...
  private classify(message: string): string {
    if (/习惯|喜欢|偏好|风格/i.test(message)) return 'user';
    if (/很好|认可|以后都这样|建议|改进/i.test(message)) return 'feedback';
    if (/禁止|必须|我们团队|因为要|为了兼容|历史遗留|设计原因|系统|项目启动|项目背景/i.test(message)) return 'project';
    if (/Jira|票号|文档在|链接|参考/i.test(message)) return 'reference';
    return 'user';
  }

  private writeMemory(message: string, memType: string): string {
...
`;
  ...
  public process(message: string): MemoryResult {
